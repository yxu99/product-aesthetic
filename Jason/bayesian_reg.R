...
ac_fit <- stan(file='regression.stan', data=stan_inputs_ac, cores=2, chains=2, iter=2000)
...
afh_fit <- stan(file='regression.stan', data=stan_inputs_afh, cores=2, chains=2, iter=2000)
draws_afh <- extract(afh_fit)
sigs_ac <- draws_ac$sigma
sigs_afh <- draws_afh$sigma
sig_diff_a <- sigs_ac - sigs_afh
cat("95% Posterior Density Interval:", quantile(sig_diff_a, 0.025), ", ", quantile(sig_diff_a, 0.975))
...
fc_fit <- stan(file='regression.stan', data=stan_inputs_fc, cores=2, chains=2, iter=2000)
...
fah_fit <- stan(file='regression.stan', data=stan_inputs_fah, cores=2, chains=2, iter=2000)
draws_fah <- extract(fah_fit)
sigs_fc <- draws_fc$sigma
sigs_fah <- draws_fah$sigma
sig_diff_a <- sigs_fc - sigs_fah
cat("95% Posterior Density Interval:", quantile(sig_diff_a, 0.025), ", ", quantile(sig_diff_a, 0.975))